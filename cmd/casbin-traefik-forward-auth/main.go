...
func main() {
	...
	// auth flags
	root.Flags().StringVar(&cfg.Auth.RouteConfigPath, "auth-route-config-path", "forward-auth-routes.yaml", "Path to the config YAML file containing route authorization rules.")
	...
	root.Flags().StringVar(&cfg.Casbin.AdapterKube.KubeConfig.Context, "casbin-adapter-kube-config-context", "", "Name of the Kubernetes context to use from the kubeconfig file.")
	root.Flags().StringVar(&cfg.Casbin.AdapterKube.KubeConfig.Namespace, "casbin-adapter-kube-config-namespace", "default", "Kubernetes namespace where Casbin policies are stored.")
	root.Flags().StringVar(&cfg.Casbin.AdapterKube.KubeConfig.Path, "casbin-adapter-kube-config-path", "", "Path to the kubeconfig file.")
