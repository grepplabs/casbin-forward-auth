...
Deploy the service with `SERVER_MODE=spoe` to run as a **SPOE Agent**. In the `spoe` mode, the `casbin-forward-auth` service sets SPOE variable `allow` in its response,
indicating whether the request is `allowed` **1** or `denied` **0**.
The `status` variable specifies the HTTP status code that HAProxy should return when a request is denied (e.g., **401** or **403**).
For `401 Unauthorized` responses, the `resp_header.www_authenticate` variable can optionally include the value for the `WWW-Authenticate` header.

HAProxy must be configured to use the SPOE Agent for request authorization.

* `haproxy.cfg`: Example backend for a service running on port 3000:
...
* `spoe-forward-rbac-auth.conf` - example SPOE agent configuration for frontend and backend events:
...
kind: Ingress
metadata:
  name: echo-haproxy-rbac
  labels:
    app: echo
  annotations:
    haproxy.org/standalone-backend: "true"
    haproxy.org/backend-config-snippet: |
      filter spoe engine forward-auth-on-backend-event config /usr/local/etc/haproxy/spoe-forward-rbac-auth.conf
      http-request deny unless { var(txn.forward_auth.allow) -m int eq 1 }
spec:
  ingressClassName: haproxy
  rules:
    - host: rbac.127.0.0.1.nip.io
      http:
        paths:
          - path: /
            pathType: Prefix
...
* Global frontend configuration example:

```haproxy
filter spoe engine forward-auth-on-frontend-event config /usr/local/etc/haproxy/spoe-forward-rbac-auth.conf
